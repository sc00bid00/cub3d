...
/*   Updated: 2023/05/22 17:19:31 by kczichow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (j < HEIGHT-1 && wall->x0 >=0 && wall->x0 <= WIDTH)
		{
			...
			if (wall->y0 >= wall->line_off && wall->y0 <= (HEIGHT - wall->line_off))
			{
				...
				if (ray->x0 - (int) ray->x0 == 0) {
					pixel = img_pixel(ray->y0 - (int)ray->y0, ((double)j / (double)wall_height), display->tex[NO]);
				} else {
					pixel = img_pixel(ray->x0- (int)ray->x0, ((double)j / (double)wall_height), display->tex[NO]);
				}

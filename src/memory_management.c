...
/*   Updated: 2023/05/03 13:46:48 by kczichow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_display(int argc, char **argv, t_display *display)
{
	(void)		argc;
	(void)		argv;
	display->mlx = mlx_init(WIDTH, HEIGTH, "Cub3d", true);
		if (!display->mlx)
			exit(EXIT_FAILURE);
	display->pos->x = 300.0;
	display->pos->y = 300.0;
	...
	display->maps->x_scaled = WIDTH / display->maps->map_x;
	display->maps->y_scaled = HEIGTH / display->maps->map_y;
}
